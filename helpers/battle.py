...
class BattleData:
    ...
    def __init__(self, teams, players, p_ids, decks, backpacks, hps, stamina, counts):
        ...
        :param teams: The teams on the battlefield that will be pitted against each other.
        ...
        :param counts: Total amount of users in the current battlefield
        """
        ...
        iter = list(range(1, 7))
        random.shuffle(iter)
        self.pps = dict(zip(iter, [ICON['ppr'], ICON['ppp'], ICON['ppw'], ICON['ppb'], ICON['ppo'], ICON['ppg']]))
    ...
    def apply_effects(self, effect, result, caster, target, extra_msg=None):
        ...
        if effect in result:
            if result[effect][1] == "self":
                ...
                self.descriptions.info[caster].append(f"{ICON['alpha']}{extra_msg}{result[effect][0]} "
    ...
    def execute_effects(self, target):
        """Actually applies the effects to the target"""
        ready_to_del = []
            ...
            if "chill" in self.effects.info[user] and "burn" in self.effects.info[user]:
                if abs(self.effects.info[user]["chill"]) > abs(self.effects.info[user]["burn"]):
                    self.descriptions.info[user].append(
                        f"{abs(self.effects.info[user]['burn'])}{ICON['burn']} {abs(self.effects.info[user]['chill'])}{ICON['chil']} cancelled out! \n{int(abs(self.effects.info[user]['chill'] - abs(self.effects.info[user]['burn']) * self.effects.info[user]['chill'] / abs(self.effects.info[user]['chill'])))}{ICON['chil']} remain")
                    ...
                    self.descriptions.info[user].append(
                        f"{abs(self.effects.info[user]['burn'])}{ICON['burn']} {abs(self.effects.info[user]['chill'])}{ICON['chil']} cancelled out! \n{int(abs(self.effects.info[user]['burn'] - abs(self.effects.info[user]['chill']) * self.effects.info[user]['burn'] / abs(self.effects.info[user]['burn'])))}{ICON['burn']} remain")
                    ...
                    self.descriptions.info[user].append(
                        f"{abs(self.effects.info[user]['burn'])}{ICON['burn']} "
                        f"{abs(self.effects.info[user]['chill'])}{ICON['chil']} cancelled out!")
                    del self.effects.info[user]["chill"]
                    del self.effects.info[user]["burn"]
            ...
            if effect == "burn":
                ...
                if str(self.effects.info[target]["recover"])[0] != "-":
                    if self.hps.info[target][0] + math.ceil(self.hps.info[target][2] / 100 * 4) <= self.hps.info[target][2]:
                        self.hps.info[target][0] += math.ceil(self.hps.info[target][2] / 100 * 4)
                        self.descriptions.info[target].append(f"{self.effects.info[target][effect]} {ICON['rec']}» {math.ceil(self.hps.info[target][2] / 100 * 4)} {ICON['hp']}")
                    else:
                ...
                if str(self.effects.info[target]["stun"])[0] != "-":
                    self.descriptions.info[target].append(
                        f"{self.effects.info[target][effect]} {ICON['stun']}» -20% acc")
                    deletion_check(effect)
                else:
                    self.effects.info[target][effect] = abs(self.effects.info[target][effect])
                ...
                if str(self.effects.info[target]["seriate"])[0] != "-":
                    self.descriptions.info[target].append(
                        f"{self.effects.info[target][effect]} {ICON['seri']}» {self.effects.info[target][effect] * 2}% {ICON['dmg']}")
                else:
                    self.effects.info[target][effect] = abs(self.effects.info[target][effect])
                ...
                if self.effects.info[target]["freeze"] != 0:
                    self.descriptions.info[target].append(f"{self.effects.info[target][effect]} {ICON['frez']}» Frozen")
                deletion_check(effect)
                    ...
                    if self.effects.info[target][effect] >= 7:
                        self.apply_effects("freeze",
                                           {"freeze": [math.floor(self.effects.info[target][effect] / 7),
                                                       "self"]},
                                           target, target,
                                           f"{math.floor(self.effects.info[target][effect] / 7) * 7} {ICON['chil']}» ")
                        self.effects.info[target][effect] = self.effects.info[target][effect] % 7
                    # self.new_line(effect_user)
                    if self.effects.info[target][effect] != 0:
                        self.descriptions.info[target].append(
                            f"{self.effects.info[target][effect]} {ICON['chil']}»")
    ...
    def execute_card_defense(self, c_level: int, c_name: str, caster, target, activation=None):
        ...
        if "sta_gain" in result:
            if random.randint(1, 100) <= round(result["acc"] + accuracy_factor):
                ...
                if not result["sta_gain"] == 1:
                    self.descriptions.info[caster].append(f"• {result['sta_gain'] - 1} {ICON['sta']}")
        ...
        if "energy_gain" in result:
            if random.randint(1, 100) <= round(result["acc"] + accuracy_factor):
                ...
                self.descriptions.info[caster].append(f"• {result['energy_gain']} {ICON['engy']}")
        # self.new_line(caster)
        if "block" in result:
            if random.randint(1, 100) <= round(result["acc"] + accuracy_factor):
                ...
                self.descriptions.info[caster].append(f"• {ICON['mi']}{ICON['ss']}")
        # self.new_line(caster)
        if "absorb" in result:
            if random.randint(1, 100) <= round(result["acc"] + accuracy_factor):
                if random.randint(1, 100) <= round(result["crit"] + crit_factor):  # / accuracy_factor):
                    ...
                    self.descriptions.info[caster].append(f"• {round(result['absorb'] * crit_multiplier * stats_factor * def_factor)} "
                                                          f"{ICON['absorb']}{ICON['crit']}» #{target}"
                                                          f"{self.pps[self.inv_teams[target]]}")
                else:
                ...
                if random.randint(1, 100) <= round(result["acc"] + accuracy_factor):
                    if random.randint(1, 100) <= round(result["crit"] + crit_factor):  # / accuracy_factor):
                        ...
                        self.descriptions.info[caster].append(
                            ...
                            f"{ICON['heal']}{ICON['crit']}» "
                            f"#{target}{self.pps[self.inv_teams[target]]}")
                    else:
                        if self.hps.info[target][0] > 0 and self.staminas.info[target] > 0:
                            self.total_damages.info[target] -= round(result["heal"] * stats_factor)
                        self.descriptions.info[caster].append(
                            f"• {round(result['heal'] * stats_factor)} {ICON['heal']}» #{target}{self.pps[self.inv_teams[target]]}")
                    self.effects_applier(result, caster, False, target)
                else:
                    self.descriptions.info[caster].append(f"• {ICON['mi']}{ICON['ss']}")
            # self.new_line(caster)
    ...
    def execute_card_offense(self, c_level: int, c_name: str, caster, target, activation=None):
                ...
                if random.randint(1, 100) <= round(result["acc"] + accuracy_factor):
                    ...
                    if random.randint(1, 100) <= round(result["crit"] + crit_factor):
                        ...
                        if self.hps.info[target][3] > round(result["tramp"]):
                            ...
                            self.descriptions.info[caster].append(
                                f"• -{round(result['tramp'])} {ICON['block']}» #{target}{self.pps[self.inv_teams[target]]}")
                        elif self.hps.info[target][4] > round(result["tramp"]):
                            self.hps.info[target][4] -= round(result["tramp"])
                            self.descriptions.info[caster].append(
                                f"• -{round(result['tramp'])} {ICON['absorb']}» #{target}{self.pps[self.inv_teams[target]]}")
                        else:
                            if 0 < self.hps.info[target][3] < round(result["tramp"]):
                                self.descriptions.info[caster].append(
                                    f"• -{self.hps.info[target][3]} {ICON['block']}» #{target}{self.pps[self.inv_teams[target]]}")
                                self.hps.info[target][3] = 0
                            elif 0 < self.hps.info[target][4] < round(result["tramp"]):
                                self.descriptions.info[caster].append(
                                    f"• -{self.hps.info[target][4]} {ICON['absorb']}» #{target}{self.pps[self.inv_teams[target]]}")
                                self.hps.info[target][4] = 0
                    ...
                    if random.randint(1, 100) <= round(result["crit"] + crit_factor) and result["damage"] != 0:
                        if (self.hps.info[target][3] < round(
                            ...
                            if not (self.hps.info[target][3] == 0 and self.hps.info[target][
                                ...
                                if self.hps.info[target][3] > 0:
                                    self.descriptions.info[caster].append(
                                        f"• {round(result['damage'] * crit_multiplier * damage_factor - self.hps.info[target][3])} {ICON['dmg']}{ICON['crit']} ({self.hps.info[target][3]} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                                elif self.hps.info[target][4] > 0:
                                    self.descriptions.info[caster].append(
                                        f"• {round(result['damage'] * crit_multiplier * damage_factor - self.hps.info[target][4])} {ICON['dmg']}{ICON['crit']} ({self.hps.info[target][4]} {ICON['absorb']})» #{target}{self.pps[self.inv_teams[target]]}")
                            else:
                                if "crit_prc" in result["mod"]:
                                    self.descriptions.info[caster].append(
                                        f"• {round(result['damage'] * crit_multiplier * damage_factor)} {ICON['pdmg']}{ICON['crit']}» #{target}{self.pps[self.inv_teams[target]]}")
                                else:
                                    self.descriptions.info[caster].append(
                                        f"• {round(result['damage'] * crit_multiplier * damage_factor)} {ICON['dmg']}{ICON['crit']}» #{target}{self.pps[self.inv_teams[target]]}")
                        else:
                            if self.hps.info[target][3] > 0:
                                blocked = True
                                self.descriptions.info[caster].append(
                                    f"• 0 {ICON['dmg']}{ICON['crit']} ({self.hps.info[target][3]} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                            elif self.hps.info[target][4] > 0:
                                self.total_damages.info[target] -= round(
                                    result["damage"] * crit_multiplier * damage_factor)
                                if self.hps.info[target][2] < self.hps.info[target][0]:
                                    self.hps.info[target][0] = self.hps.info[target][2]
                                self.descriptions.info[caster].append(
                                    f"• 0 {ICON['dmg']}{ICON['crit']} ({round(result['damage'] * crit_multiplier * damage_factor)} {ICON['absorb']})» #{target}{self.pps[self.inv_teams[target]]}")
                        if "self_damage" in result:
                            self.total_damages.info[caster] += result["self_damage"] * crit_multiplier
                            self.descriptions.info[caster].append(
                                f" {int(result['self_damage'] * crit_multiplier)} {ICON['dmg']}{ICON['crit']}» #{caster}{self.pps[self.inv_teams[caster]]}")
                        ...
                        if (self.hps.info[target][3] < round(result["damage"] * damage_factor) and
                            ...
                            if not (self.hps.info[target][3] == 0 and self.hps.info[target][4] == 0) \
                                ...
                                if self.hps.info[target][3] > 0:
                                    self.descriptions.info[caster].append(
                                        f"• {round(result['damage'] * damage_factor - self.hps.info[target][3])} {ICON['dmg']} ({self.hps.info[target][3]} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                                elif self.hps.info[target][4] > 0:
                                    self.descriptions.info[caster].append(
                                        f"• {round(result['damage'] * damage_factor - self.hps.info[target][4])} {ICON['dmg']} ({self.hps.info[target][4]} {ICON['absorb']})» #{target}{self.pps[self.inv_teams[target]]}")
                            else:
                                if "norm_prc" in result["mod"]:
                                    self.descriptions.info[caster].append(
                                        f"• {round(result['damage'] * damage_factor)} {ICON['pdmg']}» #{target}{self.pps[self.inv_teams[target]]}")
                                else:
                                    self.descriptions.info[caster].append(
                                        f"• {round(result['damage'] * damage_factor)} {ICON['dmg']}» #{target}{self.pps[self.inv_teams[target]]}")

                        else:
                            if self.hps.info[target][3] > 0:
                                blocked = True
                                self.descriptions.info[caster].append(
                                    f"• 0 {ICON['dmg']} ({self.hps.info[target][3]} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                            elif self.hps.info[target][4] > 0:
                                self.total_damages.info[target] -= round(result["damage"] * damage_factor)
                                if self.hps.info[target][2] < self.hps.info[target][0]:
                                    self.hps.info[target][0] = self.hps.info[target][2]
                                self.descriptions.info[caster].append(
                                    f"• 0 {ICON['dmg']} ({round(result['damage'] * damage_factor)} {ICON['absorb']})» #{target}{self.pps[self.inv_teams[target]]}")
                        if "self_damage" in result:
                            self.total_damages.info[caster] += result["self_damage"]
                            self.descriptions.info[caster].append(
                                f" {int(result['self_damage'])} {ICON['dmg']}» #{caster}{self.pps[self.inv_teams[caster]]}")
                    self.effects_applier(result, caster, blocked, target)
                else:
                    self.descriptions.info[caster].append(f"• {ICON['mi']}{ICON['ss']}")
                ...
                if random.randint(1, 100) <= round(result["acc"] + accuracy_factor):
                    ...
                    if random.randint(1, 100) <= round(result["crit"] + crit_factor):
                        ...
                        if self.hps.info[target][3] + self.hps.info[target][4] == 0:
                            if self.hps.info[target][3] > 0:
                                self.descriptions.info[caster].append(
                                    f"• {round(result['crush'] * crit_multiplier * damage_factor + self.hps.info[target][3] * crit_multiplier)}{ICON['dmg']}{ICON['crit']} ({self.hps.info[target][3] * crit_multiplier} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                            elif self.hps.info[target][4] > 0:
                                self.descriptions.info[caster].append(
                                    f"• {round(result['crush'] * crit_multiplier * damage_factor + self.hps.info[target][4] * crit_multiplier)}{ICON['dmg']}{ICON['crit']} ({self.hps.info[target][4] * crit_multiplier} {ICON['absorb']})» #{target}{self.pps[self.inv_teams[target]]}")
                        else:
                            self.descriptions.info[caster].append(
                                f"• {result['crush'] * crit_multiplier * damage_factor}{ICON['dmg']}{ICON['crit']}» #{target}{self.pps[self.inv_teams[target]]}")
                    else:
                        if self.hps.info[target][3] > 0:
                        ...
                        if self.hps.info[target][3] + self.hps.info[target][4] != 0:
                            if self.hps.info[target][3] > 0:
                                self.descriptions.info[caster].append(
                                    f"• {round(result['crush'] * damage_factor + self.hps.info[target][3])}{ICON['dmg']} ({round(self.hps.info[target][3] * crit_multiplier)} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                            elif self.hps.info[target][4] > 0:
                                self.descriptions.info[caster].append(
                                    f"• {round(result['crush'] * damage_factor + self.hps.info[target][4])}{ICON['dmg']} ({round(self.hps.info[target][4] * crit_multiplier)} {ICON['absorb']})» #{target}{self.pps[self.inv_teams[target]]}")
                        else:
                            self.descriptions.info[caster].append(
                                f"• {round(result['crush'] * damage_factor)}{ICON['dmg']}» #{target}{self.pps[self.inv_teams[target]]}")
                    self.effects_applier(result, caster, blocked, target)
                else:
                    self.descriptions.info[caster].append(f"• {ICON['mi']}{ICON['ss']}")
                ...
                if random.randint(1, 100) <= round(result["acc"] + accuracy_factor):
                    ...
                    if self.hps.info[target][3] < revenge_damage and self.hps.info[target][4] < revenge_damage:
                        ...
                        if not (self.hps.info[target][3] == 0 and self.hps.info[target][4] == 0):
                            if self.hps.info[target][3] > 0:
                                self.descriptions.info[caster].append(
                                    f"• {revenge_damage - self.hps.info[target][3]} {ICON['dmg']} ({self.hps.info[target][3]} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                            elif self.hps.info[target][4] > 0:
                                self.descriptions.info[caster].append(
                                    f"• {revenge_damage - self.hps.info[target][4]} {ICON['dmg']} ({self.hps.info[target][4]} {ICON['absorb']})» #{target}{self.pps[self.inv_teams[target]]}")
                        else:
                            self.descriptions.info[caster].append(
                                f"• {revenge_damage} {ICON['dmg']}» #{target}{self.pps[self.inv_teams[target]]}")

                    else:
                        if self.hps.info[target][3] > 0:
                            blocked = True
                            self.descriptions.info[caster].append(
                                f"• 0 {ICON['dmg']} ({revenge_damage} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                        elif self.hps.info[target][4] > 0:
                            self.total_damages.info[target] -= revenge_damage
                            if self.hps.info[target][2] < self.hps.info[target][0]:
                                self.hps.info[target][0] = self.hps.info[target][2]
                        ...
                        if result["eff_app"][3] == "regular" and result["eff_app"][5] == "damage":
                            if self.hps.info[target][3] < result["eff_app"][0] * crit_multiplier * abs(n_effects) \
                                ...
                                if not (self.hps.info[target][3] == 0 and self.hps.info[target][4] == 0):
                                    if self.hps.info[target][3] > 0:
                                        self.descriptions.info[caster].append(
                                            f"• {round(result['eff_app'][0] * crit_multiplier * abs(n_effects) - self.hps.info[target][3])} {ICON['dmg']}{ICON['crit']} ({self.hps.info[target][3]} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                                    elif self.hps.info[target][4] > 0:
                                        self.descriptions.info[caster].append(
                                            f"• {round(result['eff_app'][0] * crit_multiplier * abs(n_effects) - self.hps.info[target][4])} {ICON['dmg']}{ICON['crit']} ({self.hps.info[target][4]} {ICON['absorb']})» #{target}{self.pps[self.inv_teams[target]]}")
                                else:
                                    self.descriptions.info[caster].append(
                                        f"• {round(result['eff_app'][0] * crit_multiplier * abs(n_effects))} {ICON['dmg']}{ICON['crit']}» #{target}{self.pps[self.inv_teams[target]]}")
                            else:
                                if self.hps.info[target][3] > 0:
                                    self.descriptions.info[caster].append(
                                        f"• 0 {ICON['dmg']}{ICON['crit']} ({self.hps.info[target][3]} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                                    ...
                                    if self.hps.info[target][2] < self.hps.info[target][0]:
                                        self.hps.info[target][0] = self.hps.info[target][2]
                                    self.descriptions.info[caster].append(
                                        f"• 0 {ICON['dmg']}{ICON['crit']} ({result['eff_app'][0] * crit_multiplier * abs(n_effects)} {ICON['absorb']})» #{target}{self.pps[self.inv_teams[target]]}")
                            ...
                            self.total_damages.info[target] += result["eff_app"][0] * crit_multiplier * abs(
                                n_effects)
                            self.descriptions.info[caster].append(
                                f"• {round(result['eff_app'][0] * crit_multiplier * abs(n_effects))} {ICON['pdmg']}{ICON['crit']}» #{target}{self.pps[self.inv_teams[target]]}")
                        elif result['eff_app'][5] == "heal":
                            self.total_damages.info[caster] -= round(
                                result["eff_app"][0] * crit_multiplier * damage_factor * n_effects)
                            self.descriptions.info[caster].append(
                                f"• {round(result['eff_app'][0] * crit_multiplier * damage_factor * n_effects)} {ICON['heal']}{ICON['crit']}» #{caster}{self.pps[self.inv_teams[caster]]}")
                        if "self_damage" in result:
                            self.total_damages.info[caster] += result["self_damage"] * crit_multiplier
                            self.descriptions.info[caster].append(
                                f"• {int(result['self_damage'] * crit_multiplier)} {ICON['dmg']}{ICON['crit']}» #{caster}{self.pps[self.inv_teams[caster]]}")
                        ...
                        if result["eff_app"][3] == "regular" and result["eff_app"][5] == "damage":
                            if self.hps.info[target][3] < result["eff_app"][0] * abs(n_effects) and \
                                ...
                                if not (self.hps.info[target][3] == 0 and self.hps.info[target][4] == 0):
                                    if self.hps.info[target][3] > 0:
                                        self.descriptions.info[caster].append(
                                            f"• {round(result['eff_app'][0] * abs(n_effects) - self.hps.info[target][3])} {ICON['dmg']} ({self.hps.info[target][3]} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                                    elif self.hps.info[target][4] > 0:
                                        self.descriptions.info[caster].append(
                                            f"• {round(result['eff_app'][0] * abs(n_effects) - self.hps.info[target][4])} {ICON['dmg']} ({self.hps.info[target][4]} {ICON['absorb']})» #{target}{self.pps[self.inv_teams[target]]}")
                                else:
                                    self.descriptions.info[caster].append(
                                        f"• {round(result['eff_app'][0] * abs(n_effects))} {ICON['dmg']}» #{target}{self.pps[self.inv_teams[target]]}")
                            else:
                                if self.hps.info[target][3] > 0:
                                    self.descriptions.info[caster].append(
                                        f"• 0 {ICON['dmg']} ({self.hps.info[target][3]} {ICON['block']})» #{target}{self.pps[self.inv_teams[target]]}")
                                    ...
                                    if self.hps.info[target][2] < self.hps.info[target][0]:
                                        self.hps.info[target][0] = self.hps.info[target][2]
                                    self.descriptions.info[caster].append(
                                        f"• 0 {ICON['dmg']} ({result['eff_app'][0] * abs(n_effects)} {ICON['absorb']})» #{target}{self.pps[self.inv_teams[target]]}")
                        elif result["eff_app"][3] == "pierce" and result["eff_app"][5] == "damage":
                            self.hps.info[target][0] = self.hps.info[target][0] - result["eff_app"][
                                0] * abs(n_effects)
                            self.descriptions.info[caster].append(
                                f"• Dealt {round(result['eff_app'][0] * abs(n_effects))} {ICON['pdmg']}» #{target}{self.pps[self.inv_teams[target]]}")
                        elif result['eff_app'][5] == "heal":
                            self.total_damages.info[caster] -= round(
                                result["eff_app"][0] * damage_factor * n_effects)
                            self.descriptions.info[caster].append(
                                f"• {round(result['eff_app'][0] * damage_factor * n_effects)} {ICON['heal']}» #{caster}{self.pps[self.inv_teams[caster]]}")
                        if "self_damage" in result:
                            self.hp1[0] -= result["self_damage"]
                            self.descriptions.info[caster].append(
                                f"• Dealt {int(result['self_damage'])} {ICON['dmg']}» #{caster}{self.pps[self.inv_teams[caster]]}")
                    self.effects_applier(result, caster, False, target)
        ...
        if extras:
            if "self_damage" in result:
                ...
                self.descriptions.info[caster].append(
                    f"• {self_dmg} {ICON['dmg']}» #{caster}{self.pps[self.inv_teams[caster]]}")
        # self.new_line(caster)
    ...
    def execute_card_special(self, c_level: int, c_name: str, caster, target):
        ...
        if "dmg_boost" in result:
            ...
            self.descriptions.info[caster].append(f"•{ICON['efb']}» {int(round(result['dmg_boost'] * 100))}% {ICON['dmg']}")
        if "def_boost" in result:
            self.multipliers.info[caster][1] += result["def_boost"]
            self.descriptions.info[caster].append(f"•{ICON['efb']}» {int(round(result['def_boost'] * 100))}% {ICON['block']}")
        if "acc_boost" in result:
            self.multipliers.info[caster][2] += result["acc_boost"]
            self.descriptions.info[caster].append(f"•{ICON['efb']}» {int(round(result['acc_boost']))}% {ICON['eye']}")
        if "crit_boost" in result:
            self.multipliers.info[caster][3] += result["crit_boost"]
            self.descriptions.info[caster].append(f"•{ICON['efb']}» {int(round(result['crit_boost']))}% {ICON['crit']}")
    ...
    def show_hand(self) -> discord.Embed:
        ...
        if not self.turns % 4 == 0:
            embed = discord.Embed(title=None,
                                  description=f"• Type `{PREF}(card number1)(target number1)` to use card(s)")
        else:
            embed = discord.Embed(title=None,
                                  description=f"• Type `{PREF}(card number1)(target number1)` to use card(s)\n***+{math.ceil(self.turns / 4)}   {ICON['engy']} gain per turn!***")
        for i in range(len(self.players.info)):
            embed.add_field(
                name=f"__**#{i + 1}**__{self.pps[self.inv_teams[i + 1]]}{self.players.info[i + 1]}'s hand:",
                value=f"**{ICON['hp']} {self.hps.info[i + 1][0]}/{self.hps.info[i + 1][2]}** \n**{ICON['sta']} {self.staminas.info[i + 1]} {ICON['engy']} {self.stored_energies.info[i + 1]}**\n" + \
    ...
    def interpret_message(self, msg, reply_author, user):
        ...
        if not valid_play(move_numbers):
            return (f"{reply_author} that's an invalid play! "
                    f"Type your play in the format `{PREF}card#target#`, "
